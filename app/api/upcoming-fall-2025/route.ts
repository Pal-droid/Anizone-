...
export async function GET() {
  try {
    ...
    $(".widget").each((_, widgetElement) => {
      ...
      // Only target "Uscite [Season] [Year]" sections
      if (/^Uscite\s+\w+\s+\d{4}$/.test(titleText)) {
        const widgetBody = $widget.find(".widget-body");

        // Use the corrected selector
        widgetBody.find(".owl-carousel .item").each((_, element) => {
          const $item = $(element);
          ...
          if (href && imgSrc && title) {
            upcomingAnime.push({
              ...
              isOna,
              type: "upcoming",
              section: titleText, // Keep the season/year it came from
            });
          }
        });
      }
