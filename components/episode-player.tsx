...
function extractAnimeIdFromUrl(url: string): string {
  try {
    // URL format: https://www.animeworld.ac/play/anime-id or https://www.animeworld.ac/play/anime-id/episode-id
    const urlObj = new URL(url)
    const pathParts = urlObj.pathname.split("/").filter(Boolean)
    if (pathParts.length >= 2 && pathParts[0] === "play") {
      // Return just the anime ID (second part of the path)
      return pathParts[1]
    }
    // If URL doesn't match expected format, try to extract from path directly
    const pathMatch = url.match(/\/play\/([^/?#]+)/)
    if (pathMatch && pathMatch[1]) {
      return pathMatch[1]
    }
    // Last resort: return the original URL (this should not happen with proper URLs)
    ...
    ;(async () => {
      ...
      try {
        ...
        if (!sources || sources.length === 0) {
          ...
          // Try to get sources from sessionStorage as fallback
          try {
            ...
            if (storedSources) {
              ...
              if (Array.isArray(parsedSources) && parsedSources.length > 0) {
                ...
                // Use the stored sources for this request
                ...
                if (awSource || asSource) {
                  ...
                  if (asSource) {
                    const animeId = extractAnimeIdFromUrl(asSource.url)
                    params.set("AS", animeId)
                    console.log("[v0] Using anime ID from URL:", animeId, "extracted from:", asSource.url)
                  }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (!r.ok) {
          const errorText = await r.text()
          console.log("[v0] Unified stream API error response:", errorText)
          throw new Error(`Unified stream API failed with status ${r.status}: ${errorText}`)
        }

        const streamData = await r.json()
        console.log("[v0] Unified stream response:", streamData)

        const serverData = streamData[selectedServer]
        console.log("[v0] Server data for", selectedServer, ":", serverData)

        if (!serverData) {
          throw new Error(`No data returned for server ${selectedServer}`)
        }
        ...
        if (selectedServer === "AnimeWorld" && serverData.stream_url) {
          ...
          if (serverData.stream_url && serverData.stream_url.startsWith("data:text/html;base64,")) {
            try {
              // Decode the base64 HTML
              const base64Data = serverData.stream_url.split(",")[1]
              const decodedHtml = atob(base64Data)
              console.log("[v0] Decoded AnimeSaturn HTML:", decodedHtml)

              // Extract m3u8 URL from the decoded HTML
              const m3u8Match = decodedHtml.match(/videoSrc\s*=\s*['"]([^'"]+\.m3u8[^'"]*)['"]/i)
              if (m3u8Match && m3u8Match[1]) {
                const m3u8Url = m3u8Match[1]
                console.log("[v0] Extracted m3u8 URL:", m3u8Url)
                setStreamUrl(m3u8Url)
                setProxyUrl(m3u8Url) // Use m3u8 URL directly
              } else {
                // Fallback to iframe embed
                setEmbedUrl(`data:text/html;charset=utf-8,${encodeURIComponent(serverData.embed)}`)
              }
            } catch (e) {
              console.log("[v0] Failed to decode base64 HTML:", e)
              setEmbedUrl(`data:text/html;charset=utf-8,${encodeURIComponent(serverData.embed)}`)
            }
          } else if (serverData.embed.includes("data:text/html;base64")) {
            // Handle m3u8 streams with HLS.js
            setEmbedUrl(`data:text/html;charset=utf-8,${encodeURIComponent(serverData.embed)}`)
          } else {
            // Handle direct MP4 streams
                ...
                import("hls.js")
                  .then(({ default: Hls }) => {
                    ...
                    if (video && Hls.isSupported()) {
                      ...
                      console.log("[v0] HLS.js loaded for m3u8 stream")
                    }
                  })
                  .catch((err) => console.log("[v0] HLS.js load failed:", err))
              }
            }}
          />
        ) : !selectedEpisode ? (
